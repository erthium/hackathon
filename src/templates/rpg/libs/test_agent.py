...
class TestAgent(BaseAgent):
  def prepare(self, initial_state: AgentState) -> List[Item]:
    ...
    while current_money >= min([item.cost for item in PredefinedItems.ALL_ITEMS.values()]):
  ...
  def act(self, agent_state: AgentState, opponent_state: AgentState, turn: int) -> Action:
    ...
    used_item = random.choice(usable_items) if random.random() > 0.5 and len(usable_items) > 0 else None
