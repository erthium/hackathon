...
class GameEngine:
  ...
  def _validate_preparation(self, player: AgentState, requested_items: List[Item]):
    ...
    for item in requested_items:
      ...
      if item.name in checked_items and not item.is_stackable:
        raise ValueError(
          f"{player.name} tried to buy a non-stackable item multiple times: {item.name}"
        )
    ...
    self.log.add_entry(
      f"{player.name} bought items: {', '.join([item.name for item in requested_items])}."
    )
  ...
  def _apply_item_effects(self, agent_state: AgentState, item: Item) -> AgentState:
    ...
    agent_state.critical_chance = min(
      agent_state.critical_chance + item.effect.critical_chance, 1
    )
    agent_state.counter_attack_chance = min(
      agent_state.counter_attack_chance + item.effect.counter_attack_chance, 1
    )
    agent_state.double_attack_chance = min(
      agent_state.double_attack_chance + item.effect.double_attack_chance, 1
    )
    agent_state.steal_chance = min(
      agent_state.steal_chance + item.effect.steal_chance, 1
    )

    # Defensive Effects
    agent_state.defense += item.effect.defense_boost
    agent_state.prevent_death_chance = min(
      agent_state.prevent_death_chance + item.effect.prevent_death_chance, 1
    )
    agent_state.prevent_damage_chance = min(
      agent_state.prevent_damage_chance + item.effect.prevent_damage_chance, 1
    )
    agent_state.thorns += item.effect.thorns
    agent_state.health = min(
      agent_state.health + item.effect.heal_percentage * agent_state.max_health,
      agent_state.max_health,
    )
    agent_state.health = min(
      agent_state.health + item.effect.heal, agent_state.max_health
    )
    ...
    if len(agent_state.used_items) > 0:
      for index in range(len(agent_state.used_items)):
        if agent_state.used_items[index].duration > 0:
          agent_state = self._apply_item_effects(
            agent_state, agent_state.used_items[index]
          )
  ...
  def start(self, initial_money: int = 100):
    ...
    while self.agent_a_state.health > 0 and self.agent_b_state.health > 0:
      ...
      action: Action = agent.act(
        agent_state=agent_state, opponent_state=opponent_state, turn=turn
      )
      ...
      if action.action_type == ActionType.ATTACK:
        opponent_counter_attack = (
          self.roll_dice() < opponent_state.counter_attack_chance
        )
        if opponent_counter_attack:
          opponent_critical_hit = (
            2.0 if self.roll_dice() < opponent_state.critical_chance else 1.0
          )
          opponent_double_attack = (
            2 if self.roll_dice() < opponent_state.double_attack_chance else 1
          )
          attack = (
            opponent_state.attack * opponent_critical_hit * opponent_double_attack
          )
          damage = max(attack - agent_state.defense, 1)
          agent_state.health -= damage
          self.log.add_entry(
            f"{opponent_state.name} counter attacks {agent.name} for {damage} damage."
          )

        else:
          critical_hit = 2.0 if self.roll_dice() < agent_state.critical_chance else 1.0
          double_attack = (
            2 if self.roll_dice() < agent_state.double_attack_chance else 1
          )
          opponent_prevent_damage = (
            0.0 if self.roll_dice() < opponent_state.prevent_damage_chance else 1.0
          )
          attack = (
            agent_state.attack * critical_hit * double_attack * opponent_prevent_damage
          )
          damage = max(attack - opponent_state.defense, 1)
          opponent_state.health -= damage
          self.log.add_entry(
            f"{agent.name} attacks {opponent_state.name} for {damage} damage."
          )

      elif action.action_type == ActionType.STEAL:
        if len(opponent_state.items) == 0:
          self.log.add_entry(
            f"{agent.name} tries to steal from {opponent_state.name} but they have no items."
          )
        else:
          if self.roll_dice() < agent_state.steal_chance:
            ...
            self.log.add_entry(
              f"{agent.name} steals {stolen_item.name} from {opponent_state.name}."
            )
          else:
            self.log.add_entry(
              f"{agent.name} fails to steal from {opponent_state.name}."
            )
      ...
      print(
        f"Turn ended with {self.agent_a.name} having {self.agent_a_state.health} health and {self.agent_b.name} having {self.agent_b_state.health} health."
      )
      print(
        f"{self.agent_a.name} has following items: {', '.join([item.name for item in self.agent_a_state.items])}"
      )
      print(
        f"{self.agent_b.name} has following items: {', '.join([item.name for item in self.agent_b_state.items])}"
      )
      print("-" * 50)

    # Game Over
    winner: AgentState = (
      self.agent_a_state if self.agent_a_state.health > 0 else self.agent_b_state
    )
